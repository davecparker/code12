...
-- location and fire straight upwards towards the ducks.
    ...
    function start()
        ...
        -- Initialize arrays
        this.maxSize = 100
        this.bulletsArr = { length = this.maxSize }
        this.ducksArr = { length = this.maxSize }
        this.duckYStartsArr = { length = this.maxSize }
    ...
    function update()
        ...
        local j = this.ducksCount - 1; while j >= 0 do
            ...
            local duck = this.ducksArr[1+(j)]
            local duckYStart = this.duckYStartsArr[1+(j)]
            if duck.x < 0 then
        ...
        local i = this.bulletsCount - 1; while i >= 0 do
            ...
            local bullet = this.bulletsArr[1+(i)]
    ...
    function _fn.fireBullet(xStart, yStart)
        ...
        if this.bulletsCount < this.maxSize then
            ...
            bullet.ySpeed = -2
            this.bulletsArr[1+(this.bulletsCount)] = bullet
    ...
    function _fn.deleteBullet(index)
        ...
        local bullet = this.bulletsArr[1+(index)]
        bullet:delete()
        local i = index; while i < this.bulletsCount - 1 do
            
            this.bulletsArr[1+(i)] = this.bulletsArr[1+(i + 1)]
        i = i + 1; end
        this.bulletsCount = this.bulletsCount - 1
    end
    ...
    function _fn.createDuck(xStart, yStart, xSpeed)
        ...
        if this.ducksCount < this.maxSize then
            ...
            duck.xSpeed = xSpeed
            this.ducksArr[1+(this.ducksCount)] = duck
            this.duckYStartsArr[1+(this.ducksCount)] = yStart
    ...
    function _fn.deleteDuck(index)
        ...
        local duck = this.ducksArr[1+(index)]
        duck:delete()
        local i = index; while i < this.ducksCount - 1 do
            
            this.ducksArr[1+(i)] = this.ducksArr[1+(i + 1)]
            this.duckYStartsArr[1+(i)] = this.duckYStartsArr[1+(i + 1)]
