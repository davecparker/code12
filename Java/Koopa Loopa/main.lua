...
this.koopa = nil   -- koopa-troopa image
this.numberOfLevels = 8   -- how many blocks high the staircase is
...
function _fn.start()
	...
	local level = 0; while level < this.numberOfLevels do
		...
		local i = 0; while i < this.numberOfLevels + 1 - level do
			...
			block:align("bottom right"); i = i + 1
		end; level = level + 1
	end
	...
	local i = 0; while i < numberOfTiles do
		...
		local j = 0; while j < 2 do
			...
			block:align("bottom right"); j = j + 1
		end; i = i + 1
	end
	...
	if  not this.paused then
		...
		if this.koopa.x < 0 then
			...
			if this.koopa.y < yStep then
				...
				this.koopa.x = this.koopa.x - 0.2; end; end
...
function _fn.onKeyPress(keyName)
	...
	if (keyName == "space") then   -- TODO: was if ( keyName.equals("space") )
