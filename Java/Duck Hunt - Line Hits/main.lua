...
-- location and fire straight upwards towards the ducks.
    ...
    this.maxSizeBullets = 0;    -- Maximum bullets array size
    this.maxSizeDucks = 0;    -- Maximum ducks array size
    ...
    this.leftWall = nil; this.topWall = nil;    -- Lines to test line hits
    this.paused = false;    -- For starting/stopping new duck creation
    this.turboMode = false;    -- For a crazy number of ducks;
    this.frameCount = 0
    ...
    function _fn.update()
        ...
        local j = this.ducksCount - 1; while j >= 0 do
            ...
            if duck:hit(this.leftWall) then
                ...
                duck.y = ct.indexArray(this.duckYStartsArr, j) + this.amplitude * math.sin(2 * math.pi / this.period * duck.x); end   --duck.ySpeed = ct.random( -1, 1 ) / 4.0;
    ...
    function _fn.deleteBullet(index)
        ...
        local i = index; while i < this.bulletsCount - 1 do
            ct.checkArrayIndex(this.bulletsArr, i); this.bulletsArr[1+(i)] = ct.indexArray(this.bulletsArr, i + 1); i = i + 1; end
        this.bulletsCount = this.bulletsCount - 1
    end
