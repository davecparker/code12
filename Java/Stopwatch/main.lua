...
this.timeDisplay = nil   -- Display for time elapsed
this.startButton = nil   -- Button for starting the stopwatch
this.stopButton = nil   -- Button for stopping the stopwatch
this.lapButton = nil   -- Button for starting a new lap;
this.resetButton = nil   -- Button for resetting the stopwatch
this.startTime = 0   -- Start time of start buttom being pressed in milliseconds since the app was started;
this.lapStartTime = 0   -- Start time of a lap in milliseconds since the app was started;
this.pauseTime = 0   -- Start time of a pause in milliseconds since the app was started;
this.pauseLength = 0   -- How long the stopwatch has been paused in milliseconds;
this.stopwatchRunning = false   -- true when the stopwatch is running
this.lapCount = 0   -- Count of how many laps have been created
...
function _fn.update()
	...
	if this.stopwatchRunning then
		...
		if seconds < 10 then
			sec = "0" .. sec
		end; this.timeDisplay:setText(hrs .. ":" .. min .. ":" .. sec)
	end
end
...
function _fn.onMousePress(obj, x, y)
	...
	elseif obj == this.lapButton then
		...
		if this.lapCount == 1 then
			...
			if seconds < 10 then
				sec = "0" .. sec
			end; lapLengthText = hrs .. ":" .. min .. ":" .. sec; end

		local lapText = tostring(this.lapCount) .. "  " .. timeText .. " (+" .. lapLengthText .. ")"
		local lapHeight = 7   -- Height of lap display texts
		local xLap = 90
