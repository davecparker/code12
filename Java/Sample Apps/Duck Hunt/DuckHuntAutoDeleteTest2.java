// DuckHuntAutoDeleteTest2.java

// Ducks, bullets, and bombs are all put in arrays so that their deletion
// can be observed. The ducks are recycled and shouldn't be auto-deleted.
// The bullets and bombs aren't deleted by the program and should get 
// autodeleted from XSpeed, YSpeed.

// Draws the duckhunt background with the NES light gun.
// Multiple ducks are randomly generated off the left of the screen. 
// The ducks drop bombs. 
...
class DuckHuntAutoDeleteTest2
{
	final int NUM_DUCKS = 10;
	final int MAX_LENGTH = 100;
	GameObj[] ducks = new GameObj[NUM_DUCKS];
	GameObj[] bullets = new GameObj[MAX_LENGTH];
	GameObj[] bombs = new GameObj[MAX_LENGTH];
