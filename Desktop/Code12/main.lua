...
local function writeLuaCode( codeStr )
	-- Try to make the output path next to the .java file, but with .lua
	local path = sourceFile.path
	local len = string.len( path )
	if len > 5 then   -- long enough to have .java at the end
		local ext = string.sub( path, len - 4 )
		if ext == ".java" then
			local outPath = string.sub( path, 1, len - 5 ) .. ".lua"
			local outFile = io.open( outPath, "w" )
			if outFile then
				outFile:write( "-- Generated by Code12 from Java code\n" )
				outFile:write( "-- To run standalone, copy to \"Generated Lua Test\" folder and rename \"test.lua\"\n" )
